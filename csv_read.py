...
def csv_to_text(csv_file, column_name, output_file):
    ...
    print(
        f"Column '{column_name}' has been successfully written to {output_file}.")
...
filtered_emplid = [str(value)
                   for value in emplid_values if pattern_emplid.match(str(value))]

# FILTER FIRST NAMES
pattern_firstname = re.compile(r'^[A-Z][a-z]*$')
firstname_values = csv_to_list(csv_file, "Unnamed: "+str(index_of_name_first))
filtered_firstname = [
    str(value) for value in firstname_values if pattern_firstname.match(str(value))]

# FIlTER LAST NAMES
pattern_lastname = re.compile(r'^[A-Z][a-z]*(-[A-Z][a-z]*)?$')
lastname_values = csv_to_list(csv_file, "Unnamed: "+str(index_of_name_last))
filtered_lastname = [
    str(value) for value in lastname_values if pattern_lastname.match(str(value))]

# FILTER BOOKING ROOMS
pattern_booking_rooms = re.compile(r'^[A-Z]{4}-\d{4}-A\d$')
bookingroom_values = csv_to_list(
    csv_file, "Unnamed: "+str(index_of_booking_room))
filtered_bookingroom = [str(
    value) for value in bookingroom_values if pattern_booking_rooms.match(str(value))]
...
combined_list = list(zip(filtered_emplid, filtered_firstname,
                     filtered_lastname, extracted_bookingroom, number_interactions))
